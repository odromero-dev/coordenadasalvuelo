...
import 'package:sqflite/sqflite.dart' as Bdsql;

class PeticionesDB {
  static Future<void> CrearTabla(Bdsql.Database database) async {
  ...
  static Future<Bdsql.Database> db() async {
    return Bdsql.openDatabase("Coordvuelo.db", version: 1,
        onCreate: (Bdsql.Database database, int version) async {
  ...
  static Future<void> GuardarPosicion(coor, fec) async {
    ...
    await base.insert("posiciones", datos,
        conflictAlgorithm: Bdsql.ConflictAlgorithm.replace);
  }
